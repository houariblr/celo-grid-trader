...
contract GridTradingV2 is ReentrancyGuard, Ownable, IAutomationCompatible {
    ...
    // BUG-2 FIX: Per-token fee mapping — the old single uint256 conflated fees
    // from buy trades (CELO) and sell trades (cUSD) into one counter.
    ...
    function _getOraclePrice() internal view returns (uint256 price) {
        ...
        // BUG-1 FIX: Safe decimal normalisation.
        // If feedDecimals > 18 the expression `18 - feedDecimals` underflows
        // in Solidity 0.8 (arithmetic panic), permanently bricking the contract.
        if (feedDecimals <= 18) {
    ...
    /**
     * @dev BUG-5 FIX: All trade logic extracted into _executeGridInternal so that
     *      both executeGrid (external keepers) and performUpkeep (Chainlink
     *      forwarder) share a single implementation without any self-call.
     *
     *      The old performUpkeep called `this.executeGrid(...)` — an external
     *      call that shifts msg.sender to address(this).  Unless the contract
     *      itself was in isKeeper the modifier reverted every upkeep.  If the
     *      contract WAS registered, any caller could bypass the keeper ACL.
     */
    function _executeGridInternal(uint256 gridId, uint256 levelIndex) internal {
        ...
        if (level.isBuy && currentPrice <= level.price) {
            ...
            if (grid.yieldEnabled) {
                // BUG-3 FIX: capture actual amount returned (includes yield).
            ...
            amountOut -= fee;
            // BUG-2 FIX: per-token accounting (fees are in baseToken after buy)
            accumulatedFees[grid.baseToken] += fee;

            // ── Update per-grid balances ──────────────────────────────────
            ...
            uint256 fee = amountOut * feeBps / 10_000;
            amountOut -= fee;
            // BUG-2 FIX: per-token accounting (fees are in quoteToken after sell)
     ...
     *
     * @dev BUG-5 FIX: the old implementation called `this.executeGrid(...)` —
     *      an external self-call that made msg.sender == address(this) inside
     *      executeGrid.  Unless the contract was in isKeeper the ACL reverted
     *      every upkeep.  If the contract WAS registered, any account could call
     *      performUpkeep and bypass the keeper whitelist entirely.
     *
     *      Fix: enforce ACL on the actual caller (Chainlink forwarder) and call
     *      _executeGridInternal directly.  Register the forwarder address via
     *      setKeeper(forwarder, true) — the keeper whitelist is the control layer.
     */
    function performUpkeep(bytes calldata performData) external override nonReentrant {
        // BUG-5 FIX: ACL check on the real caller, not address(this)
        ...
        gridExists(gridId)
    {
        ...
        if (grid.yieldEnabled && address(moolaPool) != address(0) && grid.quoteBalance > 0) {
            // BUG-3 FIX: use actual amount returned by Moola (includes accrued yield).
    ...
    // BUG-3 FIX: Return actual amount withdrawn so callers can reconcile balances.
    // The old void signature silently discarded Moola's return value, permanently
    ...
    // The old single-counter approach mixed fees from different tokens.
