// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

/**
 * @title  GridTradingV2
 * @notice On-chain grid trading protocol on Celo.
 *         V2 — Security-hardened, Oracle-centric, Chainlink Automation ready.
 *
 * KEY CHANGES FROM V1:
 *  [CRITICAL FIX] Per-grid balance accounting — no cross-grid fund leakage
 *  [CRITICAL FIX] Price from Chainlink oracle, not keeper parameter
 *  [CRITICAL FIX] Slippage protection on all swaps
 *  [SECURITY]     ReentrancyGuard on all ERC20 transfer paths
 *  [SECURITY]     Strict input validation everywhere
 *  [FEATURE]      Chainlink Automation (checkUpkeep / performUpkeep)
 *  [FEATURE]      Moola yield on idle cUSD (opt-in per grid)
 *  [FEATURE]      Multi-keeper support with role management
 *
 * FIX (v2.1):
 *  [CRITICAL] performUpkeep previously called `this.executeGrid(...)` — an
 *             external self-call.  This has two independent security bugs:
 *
 *             (1) ACL bypass: `msg.sender` inside executeGrid becomes
 *                 `address(this)` (the contract itself).  Unless the contract
 *                 address is explicitly registered in `isKeeper`, the
 *                 `onlyKeeper` modifier REVERTS every Chainlink upkeep.
 *                 In practice teams work around this by adding the contract
 *                 to isKeeper — which then lets ANY caller trigger
 *                 performUpkeep → this.executeGrid, bypassing the
 *                 keeper whitelist entirely.
 *
 *             (2) Reentrancy surface: the external call re-enters the EVM
 *                 call stack with a fresh msg.sender context.  The
 *                 nonReentrant guard on executeGrid protects THAT frame,
 *                 but the outer performUpkeep frame has no guard and the
 *                 _status slot is NOT set when the inner call begins —
 *                 a carefully crafted token callback (e.g. a malicious
 *                 ERC-777 hook) can call back into performUpkeep before
 *                 the inner nonReentrant sets _status = ENTERED.
 *
 *             FIX: all trade logic has been extracted into
 *             `_executeGridInternal()`.  Both `executeGrid` (external
 *             keepers) and `performUpkeep` (Chainlink forwarder) call
 *             this shared internal function.  Each entry point enforces
 *             its own ACL independently and carries nonReentrant.
 *             No self-calls, no `this.`, no ACL shortcuts.
 */

import {IERC20}               from "./interfaces/IERC20.sol";
import {IMento}                from "./interfaces/IMento.sol";
import {IChainlinkFeed}        from "./interfaces/IChainlinkFeed.sol";
import {IMoolaLendingPool}     from "./interfaces/IMoola.sol";
import {IAutomationCompatible} from "./interfaces/IAutomationCompatible.sol";


// ─── Minimal OpenZeppelin ReentrancyGuard (inline) ───────────────────────────
abstract contract ReentrancyGuard {
    uint256 private constant _NOT_ENTERED = 1;
    uint256 private constant _ENTERED     = 2;
    uint256 private _status = _NOT_ENTERED;

    modifier nonReentrant() {
        require(_status != _ENTERED, "ReentrancyGuard: reentrant call");
        _status = _ENTERED;
        _;
        _status = _NOT_ENTERED;
    }
}

// ─── Minimal OpenZeppelin Ownable (inline) ────────────────────────────────────
abstract contract Ownable {
    address private _owner;
    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    constructor(address initialOwner) {
        require(initialOwner != address(0), "Ownable: zero address");
        _owner = initialOwner;
        emit OwnershipTransferred(address(0), initialOwner);
    }

    modifier onlyOwner() {
        require(msg.sender == _owner, "Ownable: not owner");
        _;
    }

    function owner() public view returns (address) { return _owner; }

    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "Ownable: zero address");
        emit OwnershipTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

// ─────────────────────────────────────────────────────────────────────────────

contract GridTradingV2 is ReentrancyGuard, Ownable, IAutomationCompatible {

    // ════════════════════════════════════════════════════════════════════════
    //  CONSTANTS
    // ════════════════════════════════════════════════════════════════════════

    /// @notice Maximum age of an oracle price before we reject it (30 minutes)
    uint256 public constant ORACLE_STALENESS_THRESHOLD = 30 minutes;

    /// @notice Default slippage tolerance: 100 = 1%, 50 = 0.5%
    uint256 public constant SLIPPAGE_BPS_DEFAULT = 100;

    uint256 public constant MAX_GRID_COUNT = 50;
    uint256 public constant MIN_GRID_COUNT = 2;

    // ════════════════════════════════════════════════════════════════════════
    //  DATA STRUCTURES
    // ════════════════════════════════════════════════════════════════════════

    struct Grid {
        address owner;
        address baseToken;
        address quoteToken;
        uint256 lowerPrice;
        uint256 upperPrice;
        uint256 gridCount;
        uint256 amountPerGrid;
        uint256 quoteBalance;
        uint256 baseBalance;
        bool    active;
        bool    yieldEnabled;
        uint256 slippageBps;
        uint256 createdAt;
    }

    struct GridLevel {
        uint256 price;
        bool    filled;
        bool    isBuy;
    }

    // ════════════════════════════════════════════════════════════════════════
    //  STATE
    // ════════════════════════════════════════════════════════════════════════

    mapping(uint256 => Grid)        public grids;
    mapping(uint256 => GridLevel[]) public gridLevels;
    mapping(address => uint256[])   public userGrids;

    uint256 public nextGridId;

    address           public mentoExchange;
    IChainlinkFeed    public priceFeed;
    IMoolaLendingPool public moolaPool;

    mapping(address => bool) public isKeeper;

    uint256 public feeBps;
    address public feeRecipient;
    // BUG-2 FIX: Per-token fee mapping.
    // The old single uint256 conflated fees from different quote tokens
    // (e.g. CELO fees from buys + cUSD fees from sells) into one counter,
    // making collectFees(token) transfer the wrong amount or the wrong token.
    mapping(address => uint256) public accumulatedFees;

    // ════════════════════════════════════════════════════════════════════════
    //  EVENTS
    // ════════════════════════════════════════════════════════════════════════

    event GridCreated(
        uint256 indexed gridId,
        address indexed owner,
        uint256 lowerPrice,
        uint256 upperPrice,
        uint256 gridCount,
        bool    yieldEnabled
    );
    event GridExecuted(
        uint256 indexed gridId,
        uint256 levelIndex,
        bool    isBuy,
        uint256 amountIn,
        uint256 amountOut,
        uint256 oraclePrice
    );
    event GridClosed(uint256 indexed gridId, address indexed owner, uint256 quoteReturned, uint256 baseReturned);
    event KeeperUpdated(address indexed keeper, bool status);
    event YieldDeposited(uint256 indexed gridId, uint256 amount);
    event YieldWithdrawn(uint256 indexed gridId, uint256 amount);
    event OracleUpdated(address indexed newFeed);
    event FeeCollected(uint256 indexed gridId, address indexed token, uint256 amount);

    // ════════════════════════════════════════════════════════════════════════
    //  MODIFIERS
    // ════════════════════════════════════════════════════════════════════════

    modifier onlyKeeper() {
        require(isKeeper[msg.sender], "GridV2: not keeper");
        _;
    }

    modifier onlyGridOwner(uint256 gridId) {
        require(grids[gridId].owner == msg.sender, "GridV2: not owner");
        _;
    }

    modifier gridExists(uint256 gridId) {
        require(gridId < nextGridId, "GridV2: grid not found");
        _;
    }

    // ════════════════════════════════════════════════════════════════════════
    //  CONSTRUCTOR
    // ════════════════════════════════════════════════════════════════════════

    constructor(
        address _keeper,
        address _mentoExchange,
        address _priceFeed,
        address _moolaPool,
        address _feeRecipient
    ) Ownable(msg.sender) {
        require(_keeper        != address(0), "zero keeper");
        require(_mentoExchange != address(0), "zero mento");
        require(_priceFeed     != address(0), "zero feed");
        require(_feeRecipient  != address(0), "zero fee recipient");

        isKeeper[_keeper] = true;
        mentoExchange     = _mentoExchange;
        priceFeed         = IChainlinkFeed(_priceFeed);
        moolaPool         = IMoolaLendingPool(_moolaPool);
        feeRecipient      = _feeRecipient;
        feeBps            = 10; // 0.1% default

        emit KeeperUpdated(_keeper, true);
    }

    // ════════════════════════════════════════════════════════════════════════
    //  ORACLE — INTERNAL
    // ════════════════════════════════════════════════════════════════════════

    function _getOraclePrice() internal view returns (uint256 price) {
        (
            uint80  roundId,
            int256  answer,
            ,
            uint256 updatedAt,
            uint80  answeredInRound
        ) = priceFeed.latestRoundData();

        require(answer > 0,                           "Oracle: non-positive price");
        require(answeredInRound >= roundId,            "Oracle: stale round");
        require(block.timestamp - updatedAt <= ORACLE_STALENESS_THRESHOLD, "Oracle: price too old");

        uint8 feedDecimals = priceFeed.decimals();
        // BUG-1 FIX: Safe decimal normalisation — guards against feeds with
        // more than 18 decimals, which would underflow and permanently revert.
        if (feedDecimals <= 18) {
            price = uint256(answer) * (10 ** (18 - feedDecimals));
        } else {
            price = uint256(answer) / (10 ** (feedDecimals - 18));
        }

    }

    function getOraclePrice() external view returns (uint256) {
        return _getOraclePrice();
    }

    // ════════════════════════════════════════════════════════════════════════
    //  SAFE ERC20 HELPERS
    // ════════════════════════════════════════════════════════════════════════

    function _safeTransfer(address token, address to, uint256 amount) internal {
        (bool ok, bytes memory data) = token.call(
            abi.encodeWithSelector(IERC20.transfer.selector, to, amount)
        );
        require(ok && (data.length == 0 || abi.decode(data, (bool))), "Transfer failed");
    }

    function _safeTransferFrom(address token, address from, address to, uint256 amount) internal {
        (bool ok, bytes memory data) = token.call(
            abi.encodeWithSelector(IERC20.transferFrom.selector, from, to, amount)
        );
        require(ok && (data.length == 0 || abi.decode(data, (bool))), "TransferFrom failed");
    }

    function _safeApprove(address token, address spender, uint256 amount) internal {
        (bool ok1,) = token.call(abi.encodeWithSelector(IERC20.approve.selector, spender, 0));
        require(ok1, "Approve reset failed");
        (bool ok2, bytes memory data) = token.call(
            abi.encodeWithSelector(IERC20.approve.selector, spender, amount)
        );
        require(ok2 && (data.length == 0 || abi.decode(data, (bool))), "Approve failed");
    }

    // ════════════════════════════════════════════════════════════════════════
    //  CREATE GRID
    // ════════════════════════════════════════════════════════════════════════

    function createGrid(
        address baseToken,
        address quoteToken,
        uint256 lowerPrice,
        uint256 upperPrice,
        uint256 gridCount,
        uint256 totalAmount,
        bool    yieldEnabled,
        uint256 slippageBps
    ) external nonReentrant returns (uint256 gridId) {
        require(baseToken  != address(0),                "GridV2: zero baseToken");
        require(quoteToken != address(0),                "GridV2: zero quoteToken");
        require(baseToken  != quoteToken,                "GridV2: same tokens");
        require(upperPrice > lowerPrice,                 "GridV2: invalid price range");
        require(upperPrice <= lowerPrice * 100,          "GridV2: range too wide (>100x)");
        require(gridCount >= MIN_GRID_COUNT
             && gridCount <= MAX_GRID_COUNT,             "GridV2: invalid grid count");
        require(totalAmount > gridCount,                 "GridV2: amount too small");
        require(slippageBps >= 10 && slippageBps <= 500, "GridV2: slippage out of range");

        _getOraclePrice();

        _safeTransferFrom(quoteToken, msg.sender, address(this), totalAmount);

        uint256 amountPerGrid = totalAmount / gridCount;

        gridId = nextGridId++;
        grids[gridId] = Grid({
            owner:         msg.sender,
            baseToken:     baseToken,
            quoteToken:    quoteToken,
            lowerPrice:    lowerPrice,
            upperPrice:    upperPrice,
            gridCount:     gridCount,
            amountPerGrid: amountPerGrid,
            quoteBalance:  totalAmount,
            baseBalance:   0,
            active:        true,
            yieldEnabled:  yieldEnabled,
            slippageBps:   slippageBps,
            createdAt:     block.timestamp
        });

        uint256 priceStep = (upperPrice - lowerPrice) / (gridCount - 1);
        for (uint256 i = 0; i < gridCount; i++) {
            gridLevels[gridId].push(GridLevel({
                price:  lowerPrice + (priceStep * i),
                filled: false,
                isBuy:  true
            }));
        }

        userGrids[msg.sender].push(gridId);

        if (yieldEnabled && address(moolaPool) != address(0)) {
            _depositToMoola(gridId, quoteToken, totalAmount);
        }

        emit GridCreated(gridId, msg.sender, lowerPrice, upperPrice, gridCount, yieldEnabled);
    }

    // ════════════════════════════════════════════════════════════════════════
    //  EXECUTE GRID  (public entry — external keepers)
    // ════════════════════════════════════════════════════════════════════════

    /**
     * @notice Execute a grid level via an external keeper.
     *         Price is sourced exclusively from Chainlink — keepers cannot
     *         influence it.
     * @param gridId     Target grid
     * @param levelIndex Which level to execute
     */
    function executeGrid(
        uint256 gridId,
        uint256 levelIndex
    ) external nonReentrant onlyKeeper gridExists(gridId) {
        _executeGridInternal(gridId, levelIndex);
    }

    // ════════════════════════════════════════════════════════════════════════
    //  EXECUTE GRID — INTERNAL LOGIC
    // ════════════════════════════════════════════════════════════════════════

    /**
     * @dev FIX [CRITICAL]: extracted from executeGrid so that both
     *      executeGrid (external keeper) and performUpkeep (Chainlink
     *      forwarder) share a single implementation without any self-call.
     *
     *      The old pattern was:
     *          function performUpkeep(...) external { this.executeGrid(...); }
     *
     *      `this.executeGrid(...)` is an external call — inside executeGrid
     *      `msg.sender` becomes `address(this)` (the contract itself).
     *      Unless the contract address is registered as a keeper, the
     *      `onlyKeeper` modifier reverts on every upkeep, breaking
     *      Chainlink automation entirely.  If the contract IS registered,
     *      the ACL is effectively bypassed: any account can call
     *      performUpkeep → this.executeGrid without being a keeper.
     *
     *      The fix: _executeGridInternal() is called directly (internal
     *      call, no msg.sender change).  executeGrid enforces onlyKeeper
     *      before calling it.  performUpkeep enforces its own check
     *      (`isKeeper[msg.sender]`) before calling it.
     *      No self-calls, no address(this) in isKeeper, no ACL gaps.
     */
    function _executeGridInternal(
        uint256 gridId,
        uint256 levelIndex
    ) internal {
        Grid storage grid = grids[gridId];
        require(grid.active, "GridV2: not active");

        GridLevel storage level = gridLevels[gridId][levelIndex];
        require(!level.filled, "GridV2: level filled");

        uint256 currentPrice = _getOraclePrice();
        uint256 amountOut;

        if (level.isBuy && currentPrice <= level.price) {
            // ── BUY: spend cUSD, receive CELO ─────────────────────────────
            uint256 amountIn = grid.amountPerGrid;
            require(grid.quoteBalance >= amountIn, "GridV2: insufficient quote balance");

            if (grid.yieldEnabled) {
                // BUG-3 FIX: Capture the actual amount withdrawn from Moola.
                // Moola can return more than `amountIn` due to accrued yield.
                // Crediting the surplus to quoteBalance prevents funds being locked forever.
                uint256 withdrawn = _withdrawFromMoola(gridId, grid.quoteToken, amountIn);
                if (withdrawn > amountIn) {
                    grid.quoteBalance += withdrawn - amountIn;
                }
            }

            uint256 minOut = (amountIn * 1e18 * (10_000 - grid.slippageBps))
                           / (currentPrice * 10_000);

            _safeApprove(grid.quoteToken, mentoExchange, amountIn);
            amountOut = IMento(mentoExchange).swapIn(
                grid.quoteToken,
                grid.baseToken,
                amountIn,
                minOut
            );

            uint256 fee = amountOut * feeBps / 10_000;
            amountOut -= fee;
            // BUG-2 FIX: accumulate into per-token mapping (fees are in baseToken after buy)
            accumulatedFees[grid.baseToken] += fee;

            grid.quoteBalance -= amountIn;
            grid.baseBalance  += amountOut;

            level.filled = true;
            level.isBuy  = false;

            emit GridExecuted(gridId, levelIndex, true, amountIn, amountOut, currentPrice);
            emit FeeCollected(gridId, grid.baseToken, fee);

        } else if (!level.isBuy && currentPrice >= level.price) {
            // ── SELL: spend CELO, receive cUSD ────────────────────────────
            uint256 celoToSell = grid.amountPerGrid * 1e18 / level.price;
            require(grid.baseBalance >= celoToSell, "GridV2: insufficient base balance");

            uint256 minOut = (celoToSell * currentPrice * (10_000 - grid.slippageBps))
                           / (1e18 * 10_000);

            _safeApprove(grid.baseToken, mentoExchange, celoToSell);
            amountOut = IMento(mentoExchange).swapIn(
                grid.baseToken,
                grid.quoteToken,
                celoToSell,
                minOut
            );

            uint256 fee = amountOut * feeBps / 10_000;
            amountOut -= fee;
            // BUG-2 FIX: accumulate into per-token mapping (fees are in quoteToken after sell)
            accumulatedFees[grid.quoteToken] += fee;

            grid.baseBalance  -= celoToSell;
            grid.quoteBalance += amountOut;

            if (grid.yieldEnabled && address(moolaPool) != address(0)) {
                _depositToMoola(gridId, grid.quoteToken, amountOut);
            }

            level.filled = true;
            level.isBuy  = true;

            emit GridExecuted(gridId, levelIndex, false, celoToSell, amountOut, currentPrice);
            emit FeeCollected(gridId, grid.quoteToken, fee);

        } else {
            revert("GridV2: price condition not met");
        }
    }

    // ════════════════════════════════════════════════════════════════════════
    //  CHAINLINK AUTOMATION
    // ════════════════════════════════════════════════════════════════════════

    /**
     * @notice Called off-chain by Chainlink nodes every block.
     * @param checkData ABI-encoded (uint256 startGridId, uint256 maxCheck)
     */
    function checkUpkeep(bytes calldata checkData)
        external
        view
        override
        returns (bool upkeepNeeded, bytes memory performData)
    {
        (uint256 startId, uint256 maxCheck) = abi.decode(checkData, (uint256, uint256));
        uint256 endId = startId + maxCheck;
        if (endId > nextGridId) endId = nextGridId;

        uint256 currentPrice;
        try priceFeed.latestRoundData() returns (
            uint80, int256 answer, uint256, uint256 updatedAt, uint80
        ) {
            if (answer > 0 && block.timestamp - updatedAt <= ORACLE_STALENESS_THRESHOLD) {
                // BUG FIX: port the safe decimal normalisation from _getOraclePrice.
                // The original `uint256(answer) * (10 ** (18 - priceFeed.decimals()))` will
                // panic with arithmetic underflow if priceFeed.decimals() > 18.
                uint8 fd = priceFeed.decimals();
                if (fd <= 18) {
                    currentPrice = uint256(answer) * (10 ** (18 - fd));
                } else {
                    currentPrice = uint256(answer) / (10 ** (fd - 18));
                }
            }
        } catch {}

        if (currentPrice == 0) return (false, "");

        for (uint256 gId = startId; gId < endId; gId++) {
            Grid storage grid = grids[gId];
            if (!grid.active) continue;

            GridLevel[] storage levels = gridLevels[gId];
            for (uint256 lIdx = 0; lIdx < levels.length; lIdx++) {
                GridLevel storage lvl = levels[lIdx];
                if (lvl.filled) continue;

                bool executable = (lvl.isBuy  && currentPrice <= lvl.price)
                               || (!lvl.isBuy && currentPrice >= lvl.price);

                if (executable) {
                    return (true, abi.encode(gId, lIdx));
                }
            }
        }
        return (false, "");
    }

    /**
     * @notice Called on-chain by the Chainlink forwarder when checkUpkeep
     *         returns true.
     *
     * @dev FIX [CRITICAL]: the old implementation called
     *      `this.executeGrid(gridId, levelIndex)` — an external self-call
     *      that made `msg.sender == address(this)` inside executeGrid,
     *      bypassing or breaking the `onlyKeeper` ACL.  See
     *      _executeGridInternal() NatSpec for the full attack analysis.
     *
     *      The Chainlink forwarder address MUST be registered as a keeper
     *      via `setKeeper(forwarder, true)` before any upkeep can execute.
     *      This is the correct security model: the keeper whitelist
     *      controls access, not an implicit contract self-registration.
     */
    function performUpkeep(bytes calldata performData)
        external
        override
        nonReentrant
    {
        // FIX: enforce keeper ACL on the actual caller (Chainlink forwarder),
        // not on address(this).  Register the forwarder via setKeeper().
        require(isKeeper[msg.sender], "GridV2: performUpkeep caller not keeper");

        (uint256 gridId, uint256 levelIndex) = abi.decode(performData, (uint256, uint256));
        require(gridId < nextGridId, "GridV2: grid not found");

        // Direct internal call — no self-call, no msg.sender shift,
        // no re-entrancy surface wider than this frame.
        _executeGridInternal(gridId, levelIndex);
    }

    // ════════════════════════════════════════════════════════════════════════
    //  CLOSE GRID
    // ════════════════════════════════════════════════════════════════════════

    function closeGrid(uint256 gridId)
        external
        nonReentrant
        onlyGridOwner(gridId)
        gridExists(gridId)
    {
        Grid storage grid = grids[gridId];
        require(grid.active, "GridV2: already closed");

        grid.active = false;

        if (grid.yieldEnabled && address(moolaPool) != address(0) && grid.quoteBalance > 0) {
            // BUG-3 FIX: Use the actual amount returned by Moola (includes accrued yield).
            // Without this, any yield earned above the deposited amount is locked in the contract.
            uint256 withdrawn = _withdrawFromMoola(gridId, grid.quoteToken, grid.quoteBalance);
            grid.quoteBalance = withdrawn;
        }

        uint256 quoteOut = grid.quoteBalance;
        uint256 baseOut  = grid.baseBalance;

        grid.quoteBalance = 0;
        grid.baseBalance  = 0;

        if (quoteOut > 0) _safeTransfer(grid.quoteToken, msg.sender, quoteOut);
        if (baseOut  > 0) _safeTransfer(grid.baseToken,  msg.sender, baseOut);

        emit GridClosed(gridId, msg.sender, quoteOut, baseOut);
    }

    // ════════════════════════════════════════════════════════════════════════
    //  MOOLA YIELD — INTERNAL
    // ════════════════════════════════════════════════════════════════════════

    function _depositToMoola(uint256 gridId, address quoteToken, uint256 amount) internal {
        _safeApprove(quoteToken, address(moolaPool), amount);
        moolaPool.deposit(quoteToken, amount, address(this), 0);
        emit YieldDeposited(gridId, amount);
    }

    // BUG-3 FIX: Return the actual amount withdrawn so callers can reconcile quoteBalance.
    // The old void signature silently discarded the Moola return value, permanently
    // locking any yield surplus accrued above the originally deposited amount.
    function _withdrawFromMoola(uint256 gridId, address quoteToken, uint256 amount)
        internal
        returns (uint256 actualWithdrawn)
    {
        actualWithdrawn = moolaPool.withdraw(quoteToken, amount, address(this));
        emit YieldWithdrawn(gridId, actualWithdrawn);
    }

    // ════════════════════════════════════════════════════════════════════════
    //  ADMIN
    // ════════════════════════════════════════════════════════════════════════

    function setKeeper(address keeper, bool status) external onlyOwner {
        require(keeper != address(0), "GridV2: zero keeper");
        isKeeper[keeper] = status;
        emit KeeperUpdated(keeper, status);
    }

    function setOracle(address newFeed) external onlyOwner {
        require(newFeed != address(0), "GridV2: zero feed");
        priceFeed = IChainlinkFeed(newFeed);
        emit OracleUpdated(newFeed);
    }

    function setFee(uint256 newFeeBps) external onlyOwner {
        require(newFeeBps <= 100, "GridV2: fee too high (>1%)");
        feeBps = newFeeBps;
    }

    // BUG-2 FIX: `token` is now the source of truth for which token to withdraw.
    // The old signature let the owner pass any token address while reading from a
    // single mixed-token counter — causing incorrect amounts or transfers of the
    // wrong token entirely.
    function collectFees(address token) external onlyOwner nonReentrant {
        uint256 amount = accumulatedFees[token];
        require(amount > 0, "GridV2: no fees for token");
        accumulatedFees[token] = 0;
        _safeTransfer(token, feeRecipient, amount);
    }

    // ════════════════════════════════════════════════════════════════════════
    //  VIEWS
    // ════════════════════════════════════════════════════════════════════════

    function getGridLevels(uint256 gridId) external view returns (GridLevel[] memory) {
        return gridLevels[gridId];
    }

    function getUserGrids(address user) external view returns (uint256[] memory) {
        return userGrids[user];
    }

    function getExecutableLevels(uint256 gridId)
        external
        view
        gridExists(gridId)
        returns (uint256[] memory executableIndexes)
    {
        uint256 currentPrice = _getOraclePrice();
        GridLevel[] storage levels = gridLevels[gridId];
        uint256[] memory tmp = new uint256[](levels.length);
        uint256 count;

        for (uint256 i = 0; i < levels.length; i++) {
            if (levels[i].filled) continue;
            bool ok = (levels[i].isBuy  && currentPrice <= levels[i].price)
                   || (!levels[i].isBuy && currentPrice >= levels[i].price);
            if (ok) tmp[count++] = i;
        }

        executableIndexes = new uint256[](count);
        for (uint256 i = 0; i < count; i++) executableIndexes[i] = tmp[i];
    }
}
